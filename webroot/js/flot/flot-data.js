...
var nbrs_total_browser = jQuery('#nbrs_total_browser').val();

var data =[];
var browsers =[];// item
var color = ["#f62d51", '#2f3d4a', "#009efb", "#55ce63", "#ffbc34", '#f5f5f5', '#4f5467', '#7460ee'];
var color_utilise = [];


for(var i=0; i<nbrs; i++){
    if (document.getElementById("navigateur_"+i)!= null && document.getElementById("session_"+i)!= null) {
        //latLng = [];
        var navigateur = document.getElementById("navigateur_" + i).value;
        ...
        obj.label = navigateur;
        obj.data = parseFloat(pageview);
        obj.color = color[i] ;
        data [navigateur]  = obj;
    }
}
...
for(var i=0; i<nbrs_total_browser; i++){
        var browser = document.getElementById("browser_" + i).value;
        var obj = new Object();
        obj.label = browser;
        obj.data = 0 ;
        if(data[browser] != undefined) {
            obj.data = data[browser]['data'];
        }
        obj.color = color[i] ;
