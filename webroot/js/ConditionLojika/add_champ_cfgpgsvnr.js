...
childs_of_bloc_champs = document.getElementById("id_listeChampAdded").childNodes.length;
  if ( childs_of_bloc_champs > 1) {
    $('.fql').show();
    $('.valTra').hide();
    console.log(childs_of_bloc_champs);
  }
...
$('#is-prise-coordonne-1').click( function(){
  if (($(this).is(':checked'))) {
    $('.klasyY').show();
    $('.klasyN').hide();
    $('.klasyBuy').hide();
...
$('#is-prise-coordonne-0').click( function(){
  if (($(this).is(':checked'))) {
    $('.klasyN').show();
    $('.klasyY').hide();
  }
...
$(document).ready(function(){
  $('.klasyN').focusout(function(){
    var rep_achat = $('#achat').val();
    if (rep_achat != "Oui") {
      $('.klasyBuy').hide();
      // alert('tsa mety');
    }else
