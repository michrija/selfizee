                        ...
                        <tbody>                            
                             ...
                             <tr>
                                    <td>
                                        <?php 
                                            if($credit->etat == 0) {
                                                echo $this->Html->link($this->Number->format($credit->credit),['action'=>'edit', $credit->id]);
                                            } 
                                            else { 
                                                echo $this->Number->format($credit->credit);
                                            }
                                        ?>
                                    </td>
                                    <td><?= $credit->client->nom ?></td>
                                    <td><?= $credit->created->format('d/m/Y') ?></td>
                                    <td><?php 
                                            $class = [0=>'text-info', 1=>'text-success', 2=>'text-danger']; 
                                            $etats = [0=>'En attente', 1=>'Validé', 2=>'Refusé']; 
                                            echo '<span class="'.$class[$credit->etat].'">'.$etats[$credit->etat].'</span>';
                                        ?>                                         
                                     </td>
                                    <td>
                                        <?php 
                                            ...
                                            if($credit->etat == 0) { ?>
                                                <?php echo $this->Html->link('<i class="mdi mdi-check-circle"></i> Valider ',['action'=>'validate', $credit->id],['escapeTitle'=>false,'aria-expanded'=>'false','class'=>'' ]); ?>
                                        <?php } } ?>
                                    </td>
                                </tr>
                             <?php endforeach; ?>
