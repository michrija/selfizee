...
<?= $this->Html->script('PageConfig/page-config.js?'.time(), ['block' => true]); ?>
				...
				<h4 class="card-title">Ajout bouton</h4>
				 <?= $this->Form->create($pageConfigBouton, ['type' => 'file', 'class' => 'col-md-6']) ?>
					<?php
						...
						echo $this->Form->control('fichier_tmp', ['type' => 'file', 'label' => 'Fichier', 'class' => 'form-control dropify', 'data-allowed-file-extensions' => 'png', 'accept' => '.png', 'data-default-file' => '/import/config_pages/boutons/'.$pageConfigBouton->fichier]);
						echo '<input id="fichier" type="hidden" value="'.$pageConfigBouton->fichier.'" name="fichier">';
					 ?>
