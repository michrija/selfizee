    ...
    <table class="table">
        ...
        foreach ($evenements as $evenement): ?>
            <?php if(!empty($evenement->photos[0]['url_thumb_popup'])){ ?>
                <tr class="red-tooltip" data-toggle="tooltip" data-placement="right" data-html="true" title="<img src='<?= $evenement->photos[0]['url_thumb_popup'] ?>' width='50' />" >
            <?php } else { ?>
                <tr>
            <?php }  ?>
                <td><?= $this->Html->link($evenement->nom, ['action' => 'board', $evenement->id]) ?></td>
