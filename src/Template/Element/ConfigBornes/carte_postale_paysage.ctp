...
<div class="row card cf_anim" id="id_cf_anim_carte_postale_paysage">
	<div class="col-sm-12 no-padding">
		<h4 class="">Carte postale paysage</h4>
	</div>
    <div class="col-md-12 cf_cadre no-padding">
        <div class="card-body no-padding-left no-padding-right no-padding-top">
            ...
            <?php if($is_new  || empty($configurationBorne->configuration_animations1[0]->cadres)) { ?> 
			<div class="cf_anim_bloc_cadre bloc_cadre_anim"  id="bloc_cadre_anim_0">                
				<div class="card-body bloc_btn_option_cadre">
					<ul class="list-inline pull-right">
						<li> <a href="#" class="btn_ajout_autre_cadre" data-owner="1"  data-typeanimation="id_cf_anim_carte_postale_paysage">Ajouter un autre cadre</a></li>
						<li> <a href="#" class="btn_suppr_cadre" id="btn_suppr_cadre_0"  data-typeanimation="1">Supprimer ce cadre</a> </li>
					</ul>
				</div>
				<div class="row kl_cadreSimple  cadre-pers">
					<div class="col-md-4 bloc_file">
						<?php 
							...
							$cadre_overlay = '';
							$cadre = [];
							if(!empty($configurationBorne->configuration_animations1[0]->cadres)){
									$cadre = $configurationBorne->configuration_animations1[0]->cadres[0];
									$fileName = $configurationBorne->configuration_animations1[0]->cadres[0]->file_name;
									if(trim($fileName) != ''){
						 ...
						 ?>
							...
							<input type="hidden" class="cadre-filename-pers" value="<?= $cadre->file_name ?>" name="configuration_animations[1][cadres][0][file_name]" />
						<?php }?>
						<?php	
							if(!empty($configurationBorne->configuration_animations1[0])){                          
								echo '<input type="hidden" name="configuration_animations[1][id]" value="'.$configurationBorne->configuration_animations1[0]->id.'">' ;                      
							} 
						?>
						<input type="hidden" name="configuration_animations[1][type_animation_id]" value="1" class="form-control">
                		<input type="hidden" name="configuration_animations[1][type_cadre]" value="0" class="form-control">
						<input type="file" name="configuration_animations[1][cadre_file][]" class="dropifyCadre" <?= !empty($urlCadreSimple) ? 'data-default-file="'.$urlCadreSimple.'"' : "" ?>   data-allowed-file-extensions="png"/>
					</div>
					<div class="col-md-4 bloc_ajout_overlay">
						<div class="label-overlay-pers">
							...
							<input type="hidden" class="cadre-overlay-pers file_overlay-pers" value="<?php echo (isset($cadre->file_overlay) && trim($cadre->file_overlay) != '') ? $cadre->file_overlay : ''; ?>" name="configuration_animations[1][cadres][][file_overlay]">
			...
			<?php } else { ?>
				<?php  
					if(!empty($configurationBorne->configuration_animations1[0]->cadres)){
						foreach($configurationBorne->configuration_animations1[0]->cadres as $ordre => $cadre){
							...
							<div class="cf_anim_bloc_cadre bloc_cadre_anim"  id="bloc_cadre_anim_<?= $ordre ?>" <?php if($ordre != 0) echo 'style="margin-top: 10px;"' ;?>>
											...
											<div class="card-body bloc_btn_option_cadre">
												<ul class="list-inline pull-right">
												<?php if($ordre == 0) echo '<li> <a href="#" class="btn_ajout_autre_cadre" data-owner="1"  data-typeanimation="id_cf_anim_carte_postale_paysage">Ajouter un autre cadre</a></li>' ; ?>
													<li> <a href="#" class="btn_suppr_cadre" id="btn_suppr_cadre_<?= $ordre ?>" data-owner="<?= $cadre->id ?>"  data-typeanimation="1">Supprimer ce cadre</a> </li>
												</ul>
											</div>
											<div class="row kl_cadreSimple  cadre-pers">
												<div class="col-md-4 bloc_file">
														<input type="hidden" value="<?= $cadre->id ?>" name="configuration_animations[1][cadres][<?= $ordre ?>][id]" />
														<input type="hidden" value="<?= $cadre->ordre ?>" name="configuration_animations[1][cadres][<?= $ordre ?>][ordre]" />
														<input type="hidden" class="cadre-filename-pers" value="<?= $cadre->file_name ?>" name="configuration_animations[1][cadres][<?= $ordre ?>][file_name]" />
												...
												<div class="col-md-4 bloc_ajout_overlay">
													<div class="label-overlay-pers">
														...
														<?php // Fichier de l'overlay ?>
														<input type="file" class="fichier_overlay-pers hide" accept="image/png">
														<input type="hidden" class="cadre-overlay-pers file_overlay-pers" value="<?php echo (isset($cadre->file_overlay) && trim($cadre->file_overlay) != '') ? $cadre->file_overlay : ''; ?>" name="configuration_animations[1][cadres][<?= $ordre ?>][file_overlay]">
