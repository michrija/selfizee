<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\Fonctionnalite[]|\Cake\Collection\CollectionInterface $fonctionnalites
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Fonctionnalite'), ['action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('List Fonctionalite Evenements'), ['controller' => 'FonctionaliteEvenements', 'action' => 'index']) ?></li>
        <li><?= $this->Html->link(__('New Fonctionalite Evenement'), ['controller' => 'FonctionaliteEvenements', 'action' => 'add']) ?></li>
    </ul>
</nav>
<div class="fonctionnalites index large-9 medium-8 columns content">
    <h3><?= __('Fonctionnalites') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('nom') ?></th>
                <th scope="col"><?= $this->Paginator->sort('titre_link') ?></th>
                <th scope="col"><?= $this->Paginator->sort('link') ?></th>
                <th scope="col"><?= $this->Paginator->sort('ordre') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($fonctionnalites as $fonctionnalite): ?>
            <tr>
                <td><?= $this->Number->format($fonctionnalite->id) ?></td>
                <td><?= h($fonctionnalite->nom) ?></td>
                <td><?= h($fonctionnalite->titre_link) ?></td>
                <td><?= h($fonctionnalite->link) ?></td>
                <td><?= $this->Number->format($fonctionnalite->ordre) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $fonctionnalite->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $fonctionnalite->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $fonctionnalite->id], ['confirm' => __('Are you sure you want to delete # {0}?', $fonctionnalite->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->first('<< ' . __('first')) ?>
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
            <?= $this->Paginator->last(__('last') . ' >>') ?>
        </ul>
        <p><?= $this->Paginator->counter(['format' => __('Page {{page}} of {{pages}}, showing {{current}} record(s) out of {{count}} total')]) ?></p>
    </div>
</div>
