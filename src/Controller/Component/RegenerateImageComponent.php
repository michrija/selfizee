...
class RegenerateImageComponent extends Component{
    const ERROR_FILE_NOT_EXIST=0;
    const IMAGE_GENERATION_METHOD = 1;
    const PNG_QUALITY=7;
    const JPEG_QUALITY = 90;
    public $_image_quality = 'jpg' ;
    ...
    { 
        ...
        // If $_image_quality is activated, the generated image will be a PNG with .jpg as a file extension.
        // This allow for higher quality and for transparency. JPG source files will also benefit from a higher quality
        // because JPG reencoding by GD, even with max quality setting, degrades the image.
        if ($this->_image_quality == 'png_all'
            || ($this->_image_quality == 'png' && $type == IMAGETYPE_PNG) && !$force_type) {
            $file_type = 'png';
        }

        if (!$src_width) {
            return !($error = ERROR_FILE_WIDTH);
        }
        if (!$dst_width) {
            $dst_width = $src_width;
        }
        ...
        $ps_image_generation_method = self::IMAGE_GENERATION_METHOD;
        ...
        $src_image = $this->create($type, $src_file);
        if ($rotate) {
        ...
        if ($dst_width >= $src_width && $dst_height >= $src_height) {
            ...
            $this->imagecopyresampled_cop($dest_image, $src_image, (int)(($dst_width - $next_width) / 2), (int)(($dst_height - $next_height) / 2), 0, 0, $next_width, $next_height, $src_width, $src_height, $quality);
        }
        $write_file = $this->write($file_type, $dest_image, $dst_file);

        @imagedestroy($src_image);
        return $write_file;
    }
    public function imagecopyresampled_cop(&$dst_image, $src_image, $dst_x, $dst_y, $src_x, $src_y, $dst_w, $dst_h, $src_w, $src_h, $quality = 3){
    ...
    public function write($type, $resource, $filename){
        $ps_png_quality = null;
        $ps_jpeg_quality = null;

        if ($ps_png_quality === null) {
            $ps_png_quality = self::PNG_QUALITY;
        }

        if ($ps_jpeg_quality === null) {
            $ps_jpeg_quality = self::JPEG_QUALITY;
        }
    ...
    function create($type, $filename){
        switch ($type) {
            ...
            case IMAGETYPE_JPEG :
            default:
               // echo " image a regenere =>".$filename."<br>"
    ...
    function resize_gif($orgfile,$newfile,$width,$height)
    {
        $this->decode($orgfile);
        ...
        foreach($this->orgvars as $i=>$arr)
        {
            ...
            //print_r($arr);
            $offset_left[$i]= isset($arr['offset_left'])?$arr['offset_left']:$arr;
            $offset_top[$i]=isset($arr['offset_top'])?$arr['offset_top']:$arr;
        }
        //$this->clearframes();
